...
-- A comment counts towards a topic when it is not hidden; it additionally
-- counts as a wild take at controversy_score >= 6. controversy_score is a
-- generated column over likes_count/dislikes_count, so reactions arrive here as
-- ordinary UPDATEs and are picked up by the same delta logic.
...
begin
  ...
  if old_topic is not distinct from new_topic then
    -- ordinary case: one topic, apply the delta only when something moved
    ...
    -- insert, delete, or a comment reparented to another topic
...
-- topics with no comments at all are not covered by the join above
update public.topics t
...
-- Same curve as before: vote volume damped by age. Refreshed on a schedule
-- rather than per read. The site's topics are admin-curated, so this touches a
-- small number of rows; if the published set ever grows into the tens of
-- thousands, narrow the WHERE clause to recently active topics.
...
-- A newly published topic should rank immediately instead of waiting for the
-- next refresh tick.
...
-- Partial rather than composite on (status, ...): every feed query filters
-- status = 'published', so indexing only those rows gives a smaller index that
-- serves the ORDER BY directly.
...
-- Backs the paginated newest-first comment reads on a topic page.
...
create or replace view public.topic_cards as
 select t.id,
    ...
    -- cast back to bigint: the previous definition produced these with count(*)
    -- and CREATE OR REPLACE VIEW cannot narrow an existing column's type
