// On an editor preview surface, broker the auth session to the parent frame over
// postMessage so the project's preview surfaces share one login; else localStorage.
//
// The broker is development-only. `import.meta.env.DEV` folds to `false` at build
// time, which makes everything past this guard unreachable, so the preview host
// names below are dropped by dead-code elimination and never reach the shipped
// production bundle. Production always uses plain localStorage.
export function brokeredPreviewStorage() {
  if (typeof window === 'undefined') return undefined;
  if (!import.meta.env.DEV) return localStorage;
